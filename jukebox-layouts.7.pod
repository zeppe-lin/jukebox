...
=head3 B<Advice for testing layouts>
...
You can then easily quit without saving by pressing ctrl-c in the
...
B<-demo> option will prevent any writing of the jukeboxrc.  You can
also use a copy of your jukeboxrc file and use the B<-cfg> option to
tell jukebox to use the copy.
...
Modifiers are one lower case letter : c for ctrl, s for shift, a for
alt and w for win/super.

The modifier(s) and key name can be preceded by a '+' to make the
shortcut a higher priority than the normal shorcuts for the focused
widget (example: "+c-f" for a control-f that overrides the default
ctrl-f action of some widgets).  Warning: It may breaks some features
or the keyboard navigation.  
